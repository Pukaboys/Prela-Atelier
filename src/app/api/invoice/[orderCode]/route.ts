...
import { formatPrice, normalizeCurrencyCode } from '@/lib/helpers'
...
export async function GET(
  ...
  const baseCurrencyOptions = await getDisplayCurrencyOptions(siteSettings)
  const requestedCurrency = request.nextUrl.searchParams.get('currency')
  const currencyOptions = requestedCurrency
    ? { ...baseCurrencyOptions, currency: normalizeCurrencyCode(requestedCurrency) }
    : baseCurrencyOptions
